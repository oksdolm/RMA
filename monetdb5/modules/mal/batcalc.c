...
mal_export str CMDbatLAPACKsignal(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci);
mal_export str CMDbatLAPACK(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci);

str
CMDbatLAPACKsignal(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	//TODO need to pass  many input and output
	// can check by pci->argc, print it to know if its total number of args

	(void) cntxt;
	(void) mb;


	fprintf(stderr, "CMDbatLAPACKsignal 1\n");
	int argsnum = pci->argc;
	int cnt1;
	//int info;
	BAT *b, *s = NULL;
	BATiter bi1;
	...
	if (argsnum>2)
	{
		bid = getArgReference_bat(stk, pci, (argsnum/2)+1);
	}
	else 
	{
		bid = getArgReference_bat(stk, pci, (argsnum/2));
	}

	b = BATdescriptor(*bid);
	CANDINIT(b, s, start, end, cnt, cand, candend);
	printf("number of rows %i\n", (int)cnt); 
	AT = (double *)calloc(argsnum*cnt, sizeof(double));
	if (AT == NULL)
	{
		fprintf(stderr, "memory for LAPACK array is not allocated");	
		return NULL;
	}

	barray = (bat **)calloc((argsnum/2)+1, sizeof(bat *));
	if (barray == NULL)
	{
		fprintf(stderr, "memory for bat array is not allocated");	
		return NULL;
	}
	...
	result = (double **)calloc((argsnum/2)+1, sizeof(double *));
	if (result == NULL)
	{
		fprintf(stderr, "memory for array of pointers to result arrays is not allocated");	
		return NULL;
	}

	//TODO use mem allocation
	//BAT **batarray[(argsnum/2)];
	//double **result[()];

	for (argit = argsnum/2; argit < argsnum; argit++)
	{

	    //fprintf(stderr, "argit is %d cnt is %d\n", argit, cnt);	
		tp1 = stk->stk[getArg(pci, argit)].vtype;
		if (tp1 == TYPE_bat || isaBatType(tp1)) {

			bid = getArgReference_bat(stk, pci, argit);
			b = BATdescriptor(*bid);

			if (b == NULL)
				return NULL;

			//must check the type for all
			if (b->ttype != TYPE_dbl )
				return NULL;

			// check each bat that it exists
			//BATcheck(b1, "CMDbatLAPACKsignal", NULL);
			...
			bi1 = bat_iterator(b);
			BATloop(b, p, q){
				double d = *(double *) BUNtail(bi1, p);
				AT[i] = d;
				//fprintf(stderr, "next %d element is %f\n", i, AT[i]);	
				i++;
			}
			BBPunfix(b->batCacheid);
		}
	...
	printf("copying to took %f seconds \n", time_taken); 

	i = 0;
	cnt1 = (int)cnt;


	/*int argsnum1 = (argsnum/2);

	double *s;
	s = (double *)calloc(cnt, sizeof(double));
	if (s == NULL)
	{
		fprintf(stderr, "memory for LAPACK array is not allocated");	
		return NULL;
	...
	info = LAPACKE_dgeqrf(LAPACK_ROW_MAJOR, cnt1, argsnum1, AT,  argsnum1, s);*/


	//here we define the size of the result
	//This is for solve operation
	//cnt1 = argsnum/2;


	for (argit = 0; argit < argsnum/2; argit++)
	{
		Barray[argit] = COLnew(TYPE_void, b->ttype, cnt1, TRANSIENT);
		result[argit] = (double*) Tloc(Barray[argit], 0);
		for(j=0;j<cnt1;j++)
		{
			result[argit][j] = AT[i];
			i++;
		}
		BATsetcount(Barray[argit], cnt1);
		BAThseqbase(Barray[argit], b->hseqbase);
		Barray[argit]->tsorted = 0;
		Barray[argit]->trevsorted = 0;
		Barray[argit]->tkey = 0;
		barray[argit] = getArgReference_bat(stk, pci, argit);
		//fprintf(stderr, "CMDbatLAPACKsignal argit is %d\n", argit);
		BBPkeepref(*barray[argit] = Barray[argit]->batCacheid);
	}
	...
	printf("copying to and back took %f seconds \n", time_taken); 
	//printf("number of final rows is %d\n", (int)cnt); 

	fprintf(stderr, "number of final rows is %d \n", cnt1);	


		//bat **batarray;
		//bat *bid;
		//batarray[0] has type bat*
		//returns pointer

     //TODO free result, barray and Barray

	fprintf(stderr, "CMDbatLAPACKsignal 7\n");
	return MAL_SUCCEED;
