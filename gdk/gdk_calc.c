...
static BUN
new_selection( BAT *b1,  const int *rgt, BAT *bn, BUN cnt, BUN start, BUN end) 
{
	BATiter bi1;
	BUN p, q;
	int i = 0;
	...
	//result = (double*) Tloc(bn, BUNfirst(bn));
	//BUNfisrt was removed, can be replaced with 0
	result = (double*) Tloc(bn, 0);

	bi1 = bat_iterator(b1);

	//TODO revrite from array pointer (like in lapack)
	BATloop(b1, p, q){
		double d = *(double *) BUNtail(bi1, p);
		   //fprintf(stderr, "FOUND BATcalcsubcst %f \n ", d);
		if (i == rgt[0]){
			val = d;
			err = FALSE;
			fprintf(stderr, "FOUND BATcalcsubcst %d,  %f\n ", i, d);
		}//new_elements1[i] = d;
		i++;
	}
	...
	if (err == TRUE){
		val = 0;
			//fprintf(stderr, "NOT FOUND BATcalcsubcst  %d\n ", i);
	}
	...
	bn->tkey = 1;
	bn->tdense = (bn->batCount <= 1 || bn->batCount == bn->batCount);
	if (bn->batCount == 1 || bn->batCount == bn->batCount)
	bn->tseqbase = bn->hseqbase;
	//bn->hsorted = 1;
	//bn->hdense = 1;
	//bn->hseqbase = 0;
	//bn->hkey = 1;
...
BATcalcsubcst(BAT *b, const ValRecord *v, BAT *s, int tp, int abort_on_error)
{
	...
	bn = COLnew(b->hseqbase, tp, 1, TRANSIENT);

	if (bn == NULL)
		return NULL;
	nils = new_selection(b, VALptr(v), bn, 1, start, end);
