...
static sql_rel * rel_matrixsolquery(mvc *sql, sql_rel *rel, symbol *q);
...
static sql_rel * rel_lapackquery(mvc *sql, sql_rel *rel, symbol *q);
...
rel_table_optname(mvc *sql, sql_rel *sq, symbol *optname)
{
	...
	//fprintf(stderr, "case rel_table_optname 1\n");
	...
	//fprintf(stderr, "case rel_table_optname 2\n");
...
query_exp_optname(mvc *sql, sql_rel *r, symbol *q)
{
	...
	case SQL_CROSS:
	{
		//fprintf(stderr, "IN query_exp_optname CROSS JOIN \n");
	...
	case SQL_MATRIXSOL:
	{
		//fprintf(stderr, "case SQL_MATRIXSOL 1\n");
		sql_rel *tq = rel_matrixsolquery(sql, r, q);

		if (!tq)
			return NULL;
		//fprintf(stderr, "case SQL_MATRIXSOL 2\n");
		return rel_table_optname(sql, tq, q->data.lval->t->data.sym);
	}
	case SQL_MATRIXQQR:
	{
	...
	case SQL_LAPACK:
	{
		//fprintf(stderr, "case SQL_LAPACK 1\n");
		sql_rel *tq = rel_lapackquery(sql, r, q);
	...
	case SQL_MATRIXCPD:
	{
		fprintf(stderr, "case SQL_MATRIXCPD \n");
...
rel_order_by_simple_column_exp(mvc *sql, sql_rel *r, symbol *column_r)
{
	...
	//fprintf(stderr, "rel_order_by_simple_column_exp -1 \n");
...
rel_order_by_column_exp(mvc *sql, sql_rel **R, symbol *column_r, int f)
{
	...
	{
		fprintf(stderr, "op value exp 6 ? %d\n", (int) test->op);
		test = test->l;
	}	
	...
	for (int i = 0; i<5 && test; i++)
	{
		fprintf(stderr, "op value exp 7 ? %d\n", (int) test->op);
		test = test->l;
	}	
			...
			if (!e && sql->session->status != -ERR_AMBIGUOUS) {
				...
				if (is_project(rel->op) && f == sql_orderby) {
					...
					if (is_groupby(g->op)) { /* check for is processed */
						//fprintf(stderr, " rel_order_by 10 \n");
				...
				if (!e)
				{
					//fprintf(stderr, " rel_order_by 11 \n");
...
//adds the complement of ap in t
static list *
...
//appends the subset of left application part 
static list *
...
static list *
append_right_appl_part(mvc *sql, list *apr, list **outexps)
{
	if (!*outexps)
		return NULL;
...
rel_matrixcpdquery(mvc *sql, sql_rel *rel, symbol *q)
{
	...
	// resolve table refs
	sql_rel *t1 = table_ref(sql, rel, tab1, 0);
	sql_rel *t2 = table_ref(sql, rel, tab4, 0);

	if (!t1 || !t2)
		return NULL;


	rel = rel_matrixcpd(sql->sa, t1, t2);
	...
	/*if (tab2) {
		sql_rel *r = rel_project(sql->sa, rel->l, sa_list(sql->sa));
		lobe = rel_order_by(sql, &r, tab2, 0);
	}

	// set orderby for right relation
	if (tab5) {
		sql_rel *r = rel_project(sql->sa, rel->r, sa_list(sql->sa));
		lobe = rel_order_by(sql, &r, tab5, 0);
	}*/
	...
	rel->nrcols = t1->nrcols + t2->nrcols;
	list *exps = new_exp_list(sql->sa);
	append_desc_part(sql, t1, rel->lexps, &exps);
	append_right_appl_part(sql, rel->rexps, &exps);
...
static sql_rel *
rel_matrixsolquery(mvc *sql, sql_rel *rel, symbol *q)
{


	//fprintf(stderr, "rel_matrixsolquery -6 \n");
	dnode *en, *n = q->data.lval->h;
	...
	rel = rel_matrixsol(sql->sa, t1, t2);
	...
	list *exps = new_exp_list(sql->sa);
	append_desc_part(sql, t1, rel->lexps, &exps);
	append_appl_part(sql, rel->rexps, rel->lexps, &exps);
	rel = rel_project(sql->sa, rel, exps);
...
rel_matrixaddquery(mvc *sql, sql_rel *rel, symbol *q)
{
	...
	if (tab5) {
		sql_rel *r = rel_project(sql->sa, rel->r, sa_list(sql->sa));
		robe = rel_order_by(sql, &r, tab5, 0);
	...
	if (tab2) {
		sql_rel *r = rel_project(sql->sa, rel->l, sa_list(sql->sa));
		lobe = rel_order_by(sql, &r, tab2, 0);
		//fprintf(stderr, "tab2 is not null\n");
...
static sql_rel *
rel_lapackquery(mvc *sql, sql_rel *rel, symbol *q)
{
	...
	rel->lord = lobe;
	//fprintf(stderr, "rel_matrixsolquery 7 \n");
	...
	//fprintf(stderr, "rel_matrixsolquery 8 \n");
...
rel_selects(mvc *sql, symbol *s)
{
	...
	case SQL_JOIN:
		...
		break;
	case SQL_CROSS:
		fprintf(stderr, "IN REL_SELECTS CROSS JOIN");
		ret = rel_crossquery(sql, NULL, s);
		sql->type = Q_TABLE;
		break;
	case SQL_MATRIXSOL:
		//fprintf(stderr, "IN REL_SELECTS");
		ret = rel_matrixsolquery(sql, NULL, s);
		sql->type = Q_TABLE;
		break;
	case SQL_UNION:
	case SQL_EXCEPT:
