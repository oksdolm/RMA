...
stmt *
stmt_atom_double(backend *be, double i)
{
	sql_subtype t;

	sql_find_subtype(&t, "double", 53, 0);
	return stmt_atom(be, atom_int(be->mvc->sa, &t, (dbl)i));
}
...
/*stmt_lapack(backend *be, list *l, int attrcnt)
{
	...
	if (q) {
		stmt *s = stmt_create(be->mvc->sa, st_lapack);
		stmt *slist = stmt_list(be->mvc->sa, l);
		s->op1 = slist;
		//s->op2 = l->h->next->data;
		//s->op3 = l->h->next->next->data;
		s->op4.lval = l;
		s->flag = attrcnt;
		s->nrcols = 1;
		s->nr = getDestVar(q);
...
stmt *
stmt_ewmul(backend *be, stmt *op1, stmt *op2)
{
	...
	q = newStmt(mb, batcalcRef, mulRef);
	q = pushArgument(mb, q, op1->nr);
	q = pushArgument(mb, q, op2->nr);

	if (q) {
		stmt *s = stmt_create(be->mvc->sa, st_ewmul);
		s->op1 = op1;
		s->op2 = op2;
		s->tname = op1->tname;
		s->cname = op1->cname;
...
stmt *
stmt_ewsub(backend *be, stmt *op1, stmt *op2)
{
	...
	q = newStmt(mb, batcalcRef, minusRef);
	q = pushArgument(mb, q, op1->nr);
	q = pushArgument(mb, q, op2->nr);

	if (q) {
		stmt *s = stmt_create(be->mvc->sa, st_ewsub);
		s->op1 = op1;
		s->op2 = op2;
		s->tname = op1->tname;
		s->cname = op1->cname;
...
stmt *
stmt_ewdiv(backend *be, stmt *op1, stmt *op2)
{
	...
	q = newStmt(mb, batcalcRef, divRef);
	q = pushArgument(mb, q, op1->nr);
	q = pushArgument(mb, q, op2->nr);

	if (q) {
		stmt *s = stmt_create(be->mvc->sa, st_ewdiv);
		s->op1 = op1;
		s->op2 = op2;
		s->tname = op1->tname;
		s->cname = op1->cname;
...
stmt *
stmt_ewadd(backend *be, stmt *op1, stmt *op2)
{
	...
	q = newStmt(mb, batcalcRef, plusRef);
	q = pushArgument(mb, q, op1->nr);
	q = pushArgument(mb, q, op2->nr);

	if (q) {
		stmt *s = stmt_create(be->mvc->sa, st_ewadd);
		s->op1 = op1;
		s->op2 = op2;
		s->tname = op1->tname;
		s->cname = op1->cname;
...
stmt_lapack(backend *be, list *l, int attrcnt)
{
	...
	int  i, j, k;
	int *c;
	c = (int *)calloc(attrcnt, sizeof(int));
	//int c[s->flag];

	q = newStmt(mb, batcalcRef, "lapack");
	fprintf(stderr, "LAPACK gencode begin\n");

	//push as many return BATs as said in the flag
	//push -1 becuse 0 arg is always reserved for output
	for (i = 1; i < attrcnt; i++)
	{
		q = pushReturn(mb, q, newTmpVariable(mb, TYPE_any));
		//fprintf(stderr, "push the result argument\n");
	}

	//push as many children BATs as given in the input statement list
	//we must first find all the children, so we need an array
	for (n = l->h, i=0; n; n = n->next, i++) 
	{
		...
		if (c[i] < 0)
		{
			fprintf(stderr, "Problem with reading the children in lapack stmt\n");
			return NULL;
		}
	}
	...
	for (j=0; j<i; j++) 
	{
		q  = pushArgument(mb, q, c[j]);
		fprintf(stderr, "push the input argument %d\n", c[j]);
	}

	k = getDestVar(q);

	//rename output results. st_result use the exact name in search
	//for (i = 1; i < attrcnt; i++)
	//{
		//resi = getArg(q, i);
		//fprintf(stderr, "renaming res variable to r%d_%d\n", i, s->nr);
		//renameVariableExt(mb, resi, "r%d_%d", i, k);
	//}

	if (k) {
		stmt *s = stmt_create(be->mvc->sa, st_lapack);
		stmt *slist = stmt_list(be, l);
		s->op1 = slist;
		s->op4.lval = l;
		s->flag = attrcnt;
		s->nrcols = 1;
...
stmt *
stmt_norm(backend *be, stmt *op1)
{

	InstrPtr q = NULL, p = NULL, w = NULL, v = NULL;
	MalBlkPtr mb = be->mb;
	int k;
	...
	w = newStmt(mb, aggrRef, sumRef);

	w = pushArgument(mb, w, k); 

	setVarType(mb, getArg(w, 0), TYPE_dbl);
	setVarUDFtype(mb, getArg(w, 0));

	k = getDestVar(w);

	v = newStmt(mb, mmathRef, "sqrt");

	v = pushArgument(mb, v, k); 

	k = getDestVar(v);

	p = newStmt(mb, batcalcRef, divRef);

	p = pushArgument(mb,  p, op1->nr);
	p = pushArgument(mb,  p, k);

	k = getDestVar(p);

	if (p) {
		stmt *s = stmt_create(be->mvc->sa, st_norm);
		s->op1 = op1;
		s->tname = op1->tname;
		s->cname = op1->cname;
		s->nrcols = 1;
...
stmt *
stmt_innorm(backend *be, stmt *op1, stmt *op2)
{

	InstrPtr q = NULL, v = NULL, w = NULL, p = NULL;
	MalBlkPtr mb = be->mb;
	int k;
	...
	p = newStmt(mb, batcalcRef, mulRef);
	...
	v = newStmt(mb, batcalcRef, minusRef);

	v = pushArgument(mb, v, op1->nr); 
	v = pushArgument(mb, v, k);

	if (v) {
		stmt *s = stmt_create(be->mvc->sa, st_innorm);
		s->op1 = op1;
		s->op2 = op2;
		s->tname = op1->tname;
		s->cname = op1->cname;
...
stmt_sum(backend *be, stmt *op1)
{
	...
	q = newStmt(mb, aggrRef, sumRef);
	q = pushArgument(mb, q, op1->nr);

	if (q) {
		stmt *s = stmt_create(be->mvc->sa, st_sum);
		s->op1 = op1;
		s->nrcols = 1;
		s->tname = op1->tname;
		s->cname = op1->cname;
...
stmt *
stmt_sel(backend *be, stmt *st, stmt *st1)
{
	...
	q = newStmt(mb, batcalcRef, minusRef);
	q = pushArgument(mb, q, st->nr);
	q = pushArgument(mb, q, st1->nr);


	if (q) {
		stmt *s = stmt_create(be->mvc->sa, st_sel);
		s->op1 = st;
		s->op2 = st1;
		s->nrcols = 1;
		s->nr = getDestVar(q);
