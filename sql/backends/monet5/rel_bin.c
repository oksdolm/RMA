...
static void
split_exps_appl_desc(backend *be, stmt *p, list *exps, list **a, list **d)
{
...
static stmt *
gen_orderby_ids(backend *be, stmt *s, list *ord)
{
	...
	stmt_set_nrcols(psub);
	//fprintf(stderr, ">>> [gen_orderby_ids] 1 \n ");


	// ordering of the order specification columns to know the final order of OIDs for
	for (n = ord->h; n; n = n->next) {
		//fprintf(stderr, ">>> [gen_orderby_ids] 2 in loop \n");
		stmt *orderby = NULL;
		sql_exp *orderbycole = n->data;
		stmt *orderbycolstmt = exp_bin(be, orderbycole, s, psub, NULL, NULL, NULL, NULL);

		if (!orderbycolstmt) {
			//fprintf(stderr, ">>> [gen_orderby_ids] !orderbycolstmt\n");
			assert(0);
			return NULL;
		}
		...
		// single values don't need sorting 
		if (orderbycolstmt->nrcols == 0) {
			//fprintf(stderr, ">>> [gen_orderby_ids] orderbycolstmt->nrcols == 0\n");
			orderby_ids = NULL;
			break;
		}
...
static void
align_by_ids(backend *be, stmt *orderby_ids, list *l, list **ol)
{
	...
	for(n = l->h; n; n = n->next) {
		...
		if (orderby_ids)
		{
			s = stmt_project(be, orderby_ids, c);
			//fprintf(stderr, ">>> orderby_ids is not empty\n");
...
static stmt *
rel2bin_matrixcpd(backend *be, sql_rel *rel, list *refs)
{
	...
	split_exps_appl_desc(be, left, rel->lexps, &la, &ld);
	split_exps_appl_desc(be, right, rel->rexps, &ra, &rd);

	// generate the orderby ids
	orderby_idsl = gen_orderby_ids(be, left, rel->lord);
	//orderby_idsr = gen_orderby_ids(be, right, rel->rord);
	//fprintf(stderr, " matrix cpd begin");

	// align lists according to the orderby ids
	align_by_ids(be, orderby_idsl, ld, &l);
	align_by_ids(be, orderby_idsr, rd, &l);
	align_by_ids(be, orderby_idsl, la, &loa);
	align_by_ids(be, orderby_idsr, ra, &roa);
	//fprintf(stderr, " matrix cpd mid");


    ///////////////////////////////////////////////////

	for(nl = ld->h; nl; nl = nl->next)
	{
		stmt *c = nl->data;
		stmt *s;
		stmt *s1;
		...
		//fprintf(stderr, "    %c: descriptive left column name\n", *cname);

		s = stmt_atom_string(be, column_name(sql->sa, loa->h->data));
		s = stmt_alias(be, s, tname, cname);
		s1 = stmt_temp(be, tail_type(s));	
		s1->tname = s->tname;
		s1->cname = s->cname;
		s1 = stmt_append(be, s1, s);	

		for(j = loa->h->next; j; j = j->next)
		{
			stmt *cn = j->data;
			stmt *s2;
			const char *cnch = column_name(sql->sa, cn);

			//fprintf(stderr, "    %c: column name\n", *cnch);

			s2 = stmt_atom_string(be, cnch);
			s2 = stmt_alias(be, s2, tname, cname);
			s1 = stmt_append(be, s1, s2);	
		}

		list_append(l, s1);
	}

	// must be crossproduct At*B, so outer loop on B
	for(nl = roa->h; nl; nl = nl->next){
		...
		c = nl->data;
		c1 = loa->h->data;
		s = stmt_ewmul(be, c, c1);
		s = stmt_sum(be, s);
		s = const_column(be, s);
			for(j = loa->h->next; j; j = j->next)
			{
				stmt *c1 = j->data;
				s1 = stmt_ewmul(be, c, c1);
				s1 = stmt_sum(be, s1);
				s = stmt_append(be, s, s1);
			}
		list_append(l, s);
	}
...
static stmt *
rel2bin_matrixadd(backend *be, sql_rel *rel, list *refs)
{

	//fprintf(stderr, "rel2bin_matrixsol  1\n");
	mvc *sql = be->mvc;
	// list of all statements (result)
	list *l;
	...
	// construct result list of statements
	l = sa_list(sql->sa);
	//unordered application and descriptive parts
	la = sa_list(sql->sa);
	ra = sa_list(sql->sa);
	ld = sa_list(sql->sa);
	rd = sa_list(sql->sa);
	//ordered application parts
	loa = sa_list(sql->sa);
	roa = sa_list(sql->sa);
	...
	for(nl = loa->h, nr = roa->h; nl&&nr; nl = nl->next, nr = nr->next)
	{
		stmt *lc = nl->data;
		stmt *rc = nr->data;
		stmt *s;
		s = stmt_ewadd(be, lc, rc);
		list_append(l, s);
...
static stmt *
rel2bin_matrixqqr(backend *be, sql_rel *rel, list *refs)
{
	...
	for(n = loa->h; n; n = n->next)
	{
		...
		n->data = s;
			...
			for(j = n->next; j; j = j->next)
			{
				stmt *c1 = j->data;
				stmt *s1 = stmt_innorm(be, c1, s);
				s1->tname = c1->tname;
				s1->cname = c1->cname;
				j->data = s1;
				//list_append(l, s1);
...
static stmt *
rel2bin_lapack(backend *be, sql_rel *rel, list *refs)
{
	...
	stmt *lapack = NULL;
	stmt *st = NULL;
	//interator for unlisting the result to the BATs
	int i;
	//names for columns 
	...
	// copy to LAPACK implementation
    //number of elements in application part
	for (n = loa->h; n; n = n->next) {
		//stmt *s = n->data;
		count++;
		//fprintf(stderr, "number of elements in loa %d\n", count);
	}


	//lapack stmt that takes list of statement and its length 
	lapack = stmt_lapack(be, loa, count);
	st = loa->h->data;
	tname = table_name(sql->sa, st);
	cname = column_name(sql->sa, st);
	lapack = stmt_alias(be, lapack, tname, cname);

	list_append(l, lapack);
    //delisting the result to standalone BATs
	for (i = 1, n = loa->h->next; i<count && n; i++, n = n->next)
	{
		stmt *c = n->data;
		stmt *s = stmt_result(be, lapack, i);
		tname = table_name(sql->sa, c);
	    cname = column_name(sql->sa, c);
		s = stmt_alias(be, s, tname, cname);
		list_append(l, s);
	}
	return stmt_list(be, l);
...
static stmt *
rel2bin_matrixsol(backend *be, sql_rel *rel, list *refs)
{
	...
	templ = sa_list(sql->sa);
	templ1 = sa_list(sql->sa);
	...
	list_merge(l, loa, NULL);
	//list_merge(l, roa, NULL);

	// TODO fix it, only one additional column should be added
	for(nl = ld->h; nl; nl = nl->next)
	{
		stmt *c = nl->data;
		stmt *s;
	...
	//crossproduct At*y, left and right relations are used

	i = 1;

	y = roa->h->data;
	//ycname = column_name(sql->sa, y);
	//ytname = table_name(sql->sa, y);

	aty = stmt_ewmul(be, y, loa->h->data);
	aty = stmt_sum(be, aty);
	aty = const_column(be, aty);
	fprintf(stderr, " first row in aty \n");

	for(nl = loa->h->next; nl; nl = nl->next){
		i++;
		...
		s = stmt_ewmul(be, y, c);
		s = stmt_sum(be, s);
		aty = stmt_append(be, aty, s);
		fprintf(stderr, " %d row in aty \n", i);

	}
	//list_append(l, aty);

	//crossproduct At*A, just left relation is used

	i = 0;
	for(nl = loa->h; nl; nl = nl->next){
		...
		const char *cnch = column_name(sql->sa, s);
		fprintf(stderr, "    %c: column name\n", *cnch);
			...
			for(j = loa->h->next; j; j = j->next)
			{
				...
				const char *cnch1 = column_name(sql->sa, s);
				fprintf(stderr, "    %c: column name in loop\n", *cnch1);
			}
		//comment to test creation of an id matrix
		list_append(templ, s);
		i++;
	}
	//list_merge(l, templ, NULL);


	lit = 0;
	//Identity matrix that is turned to an inv 
	for(nl = loa->h; nl; nl = nl->next)
	//for(l = 1; l <=i; l++)
	{
		stmt *s;
		...
		if (lit == 1)
		{
			s3 = stmt_atom_double(be, 1.0);		
		}
		else
		{
			s3 = stmt_atom_double(be, 0.0);
		}

		s3 = stmt_alias(be, s3, tname, cname );
		s = stmt_temp(be, tail_type(s3));	
		s->tname = s3->tname;
		s->cname = s3->cname;
		s = stmt_append(be, s, s3);
		fprintf(stderr, " first row, %d column in id matrix \n", lit);
		...
		for(k = 2; k <=i; k++)
		{
			...
			if (lit == k)
			{
				s1 = stmt_atom_double(be, 1.0);
			}
			else
			{
				s1 = stmt_atom_double(be, 0.0);
			}
			s1 = stmt_alias(be, s1, tname, cname);
			s = stmt_append(be, s, s1);
			fprintf(stderr, " %d row, %d column in id matrix \n", k, lit);

		}	
		list_append(templ1, s);
	}
	//list_merge(l, templ1, NULL);

	// inversion (At*A)^(-1)
	i = 0;
	it1 = 1;
	it2 = 0;
	for(nl = templ->h, nr = templ1->h; nl&&nr; nl = nl->next, nr = nr->next)
	{
		stmt *c = nl->data;
		stmt *rc = nr->data;
		stmt *s1;
		...
		s1 =  stmt_atom_int(be, i);
		s2 = stmt_sel(be, c, s1);
		s2 = stmt_sum(be, s2);
		s3 = stmt_ewdiv(be, c, s2);
		

		rs3 = stmt_ewdiv(be, rc, s2);
		nl->data = s3;
		nr->data = rs3;

		it2 = 1;
		//fprintf(stderr, " it1 = %d \n", it1);
		for(nl1 = templ->h, nr1 = templ1->h; nl1&&nr1; nl1 = nl1->next, nr1 = nr1->next)
		{
			stmt *c1;
			stmt *c2;
			stmt *c3;
			...
			//fprintf(stderr, " it2 = %d \n", it2);
			if (it1 != it2)
			{
				//fprintf(stderr, " it1 != it2\n");
				c1 = nl1->data;
				rc1 = nr1->data;
				c2 = stmt_atom_int(be, i);
				c2 = stmt_sel(be, c1, c2);
				c2 = stmt_sum(be, c2);

				c3 = stmt_ewmul(be, c2, s3);
				c4 = stmt_ewsub(be, c1, c3);
				

				rc3 = stmt_ewmul(be, c2, rs3);
				rc4 = stmt_ewsub(be, rc1, rc3);
	...
	//final crossproduct (At*A)^(-1)*(At*y), e.g. templ1*aty

	y = templ1->h->data;
	inaty = stmt_ewmul(be, y, aty);
	inaty = stmt_sum(be, inaty);
	inaty = const_column(be, inaty);

	// set proper names to inaty 
	y = roa->h->data;
	ycname = column_name(sql->sa, y);
	ytname = table_name(sql->sa, y);
	inaty  = stmt_alias(be, inaty, ycname, ytname);
	inaty->cname = ycname;
	inaty->tname = ytname;

	for(nl = templ1->h->next; nl; nl = nl->next){

		stmt *c;
		stmt *s;

		c = nl->data;
		s = stmt_ewmul(be, c, aty);
		s = stmt_sum(be, s);
		s->cname = ycname;
		s->tname = ytname;
		inaty = stmt_append(be, inaty, s);
	}
...
subrel_bin(backend *be, sql_rel *rel, list *refs) 
{
	...
	case op_ddl:
		...
		break;
	case op_matrixsol:
		//fprintf(stderr, ">>> [subrel_bin_sol]\n");
		s = rel2bin_matrixsol(be, rel, refs);
		//fprintf(stderr, ">>> END: [subrel_bin_sol]\n");
		break;
	case op_matrixcpd:
		//fprintf(stderr, ">>> [subrel_bin_sol]\n");
		s = rel2bin_matrixcpd(be, rel, refs);
		//fprintf(stderr, ">>> END: [subrel_bin_sol]\n");
		break;
	case op_matrixadd:
		//fprintf(stderr, ">>> [subrel_bin_sol]\n");
		s = rel2bin_matrixadd(be, rel, refs);
		//fprintf(stderr, ">>> END: [subrel_bin_add]\n");
		break;
	case op_matrixqqr:
		//fprintf(stderr, ">>> [subrel_bin_sol]\n");
		s = rel2bin_matrixqqr(be, rel, refs);
		//fprintf(stderr, ">>> END: [subrel_bin_qqr]\n");
		break;
	case op_lapack:
		fprintf(stderr, ">>> [subrel_bin lapack]\n");
		//must be lapack later
		s = rel2bin_lapack(be, rel, refs);
		//fprintf(stderr, ">>> END: [subrel_bin lapack]\n");
		break;
	}
	if (s && rel_is_ref(rel)) {
